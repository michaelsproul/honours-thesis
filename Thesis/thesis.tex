...
The unit type, $\mathcal{I}$ is inhabited by a single term $*$. The term \b{let $*$ be $t$ in $u$} allows for the \i{elimination} of a term $t$ of type $I$, as shown by the rule $(Unit-E)$.

% The terms denoted by meta-variable $v$ are the \i{values}. Values are terms of the language that we consider to be fully/completely evaluated.
