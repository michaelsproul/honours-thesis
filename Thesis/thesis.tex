...
In order to state the progress and preservation lemmas, Coq also needs an idea of which terms are considered \textit{values}, i.e. those terms in a form that can not be simplified further. For this, we use an inductive predicate \texttt{value t} with the following definition:

\begin{verbatim}
Inductive value :=
\end{verbatim}

Note that the type of \texttt{value} is \texttt{term -> Prop}. Given a \texttt{term}, \texttt{t}, a Coq term with type \texttt{value t} \textit{witnesses} the truth of the proposition \texttt{value t}, which itself has type \texttt{Prop}. This is in contrast to \texttt{ty} and \texttt{term} which have type \texttt{Set}. Types in Prop are intended to represent proof terms, while those in Set are meant to represent data. Types in Prop don't work with Coq's program extraction, but this is no concern for our purposes. The real reason to embrace the distinction is to minimise friction with Coq's standard library, which provides definitions for basic logical connectives operating only on \texttt{Prop}s.
...
First, we must inform DbLib which of our term constructors is for variables. DbLib allows the types of \textit{values} (\texttt{V}) and \textit{terms} (\texttt{T}) to differ, but we don't make use of this capability, instead using \texttt{term}s everywhere and the \texttt{value} predicate. The type-class has the following definition in DbLib:
...
The only binders in our linear lambda calculus are lambda abstractions, so our implementation of traverse simply has to increment \c{l} when recursing below one. This is the same as for the simply typed lambda calculus.
