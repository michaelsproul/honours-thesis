...
For the semantics of our language we use call-by-value small step semantics. We represent the semantic rules in Coq as an inductive relation \c{step : term -> term -> Prop} so that if \c{step e e'} is true, \c{e'} is the result of partially evaluating \c{e}.
...
The first rule, \c{StepAppAbs} is the familiar $\beta$-rule for evaluating the application of an abstraction to a value. Note the use of DbLib's \c{subst} function to create the result of the computation. In mathematical notation the rule is: $(\lambda 0 : \tau. e) \; v \Longrightarrow e[v/0]$.
